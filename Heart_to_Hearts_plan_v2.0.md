...
| **자연스러운 바이럴** | 꼬리물기 방식의 챌린지 전파 구조 자체가 곧 바이럴 루프입니다. 한 명이 참여하면 반드시 다음 한 명을 초대해야 하므로, 별도의 마케팅 비용 없이도 사용자 기반이 기하급수적으로 확장됩니다. |
...
### 1.5. 핵심 메커니즘 요약 (Host·초대 구조)

**생성자 = 심판(Host)**  
챌린지를 시작하는 사람은 "질문을 던지는 사람"일 뿐이며, 자신의 마음이 노출되지 않습니다. 첫 참여자는 **초대**로 3~5명을 동시에 지정하고, 각 갈래에서 체인이 뻗어 나갑니다.

| 단계 | 행위자 | 행위 | 결과 |
| :--- | :--- | :--- | :--- |
| 1 | A (Host·생성자) | 질문을 선택하고 **첫 참여자 3~5명(B, C, D, E)**을 **초대**합니다. (지목이 아닌 "이 질문에 답할 사람을 부른다"는 개념) | B, C, D, E 각각에게 "누군가 이 질문을 던졌어요!" 푸시 알림이 발송됩니다. |
| 2 | B, C, D, E (초대받은 사람) | 푸시 알림을 통해 앱에 진입하고, 질문에 대한 답으로 **각자** 누군가를 지목합니다. 누가 질문을 던졌는지( Host)는 알 수 없습니다. | 각 갈래에서 체인이 시작됩니다. (B→?, C→?, D→?, E→? …) |
| 3-A | B | B가 **F를 지목**한 경우 (일반 지목) | F에게 푸시 알림이 발송되고, 해당 체인이 이어집니다. |
| 3-B | B | B가 **A(Host)를 지목**한 경우 (상호 지목 성립) | B에게 편지 작성 화면이 표시됩니다. A도 이 시점에서 체인에 참여자로 포함됩니다. |
| 4 | B | (상호 지목 시) A에게 보낼 익명 편지를 작성하고, 체인을 이어갈 다음 주자를 지목합니다. | A에게 편지가 전달되고, 다음 주자에게 푸시 알림이 발송됩니다. |
| 5 | 시스템 | 챌린지 시작 후 **48시간이 경과**하거나, **참여자가 30명에 도달**하면(30번째 참여자가 다음 주자를 지목한 시점) 챌린지가 종료됩니다. | 챌린지가 종료되고, 모든 참여자에게 결과 알림이 발송됩니다. 30명 도달 시 "이번 챌린지는 30명이 모두 참여하여 종료되었습니다." 메시지와 함께 즉시 마감·결과 공개됩니다. |
| 6 | 모든 참여자 | 결과 보관함에서 나에 대해 작성된 편지의 **내용만** 확인합니다. 작성자는 익명입니다. | (분기 발생) |
| 7-A | 참여자 | 작성자 1명을 알고 싶은 경우, **1,052원**을 결제합니다. | 해당 편지 작성자의 신원이 공개됩니다. |
| 7-B | 참여자 | 모든 작성자를 알고 싶은 경우, **5,260원** (1,052원 x 5)을 결제합니다. | 모든 편지 작성자의 신원이 공개됩니다. |

**구조의 장점**  
- Host의 마음이 노출되지 않아 부담이 적습니다.  
- 3~5명에게 동시 초대하므로 바이럴 계수가 기존 대비 3~5배로 증가합니다.  
- 여러 체인이 동시에 뻗어 나가 상호 지목 발생 확률이 높아져, "마음이 통했어요!" 경험이 더 자주 일어납니다.  
- Host도 체인 속에서 누군가에게 지목받을 수 있어, 참여 동기가 유지됩니다.
...
| 용어 | 영문명 | 정의 |
| :--- | :--- | :--- |
| **챌린지** | Challenge | 사용자(Host)가 특정 질문을 선택하고 **첫 참여자 3~5명을 초대**하면서 시작되는 48시간 한정 이벤트 단위입니다. 하나의 챌린지 안에는 여러 갈래의 체인이 공존할 수 있으며, **참여 인원은 최대 30명**으로 제한됩니다. |
| **Host(심판)** | Host | 챌린지를 생성한 사용자입니다. 질문을 선택하고 첫 참여자 3~5명을 **초대**할 뿐, "누군가를 좋아해서 지목하는" 역할이 아닙니다. 인당 최대 3회까지 챌린지를 생성할 수 있으며, 체인에서 누군가에게 지목받으면 그때 참여자로 포함됩니다. |
| **초대** | Invitation | Host가 챌린지 시작 시 "이 질문에 답할 사람"으로 지정하는 3~5명입니다. 초대받은 사람에게는 "누군가 이 질문을 던졌어요!" 알림이 가며, 누가 던졌는지(Host)는 알 수 없습니다. |
| **챌린지 체인** | Challenge Chain | 한 챌린지 내에서 한 갈래가 B→C→... 로 이어지는 지목의 연결 고리입니다. 한 챌린지에 여러 체인(여러 갈래)이 동시에 존재할 수 있습니다. |
| **참여자** | Participant | 챌린지에 초대받거나 지목되어 참여한 모든 사용자입니다. Host는 누군가에게 지목받기 전까지는 참여자 수에 포함되지 않을 수 있으며, 지목받은 시점부터 참여자로 간주됩니다. |
| **지목** | Nomination | (초대받은 사람 또는 체인 내 참여자가) 주어진 질문에 대한 답으로 특정 친구 1명을 선택하는 행위입니다. |
| **피지목자** | Nominee | 다른 참여자에 의해 지목된 사용자입니다. 피지목자는 자신을 누가 지목했는지 알 수 없습니다. |
| **상호 지목** | Mutual Nomination | A가 B를 지목하고, B 또한 A를 지목한 경우를 의미합니다. 이 경우 편지 작성 이벤트가 발생합니다. |
| **편지** | Letter | 상호 지목이 성립되었을 때, 지목한 상대방에게 익명으로 보낼 수 있는 텍스트 메시지입니다. |
| **질문 세트** | Question Set | 챌린지 생성 시 Host가 선택할 수 있는 질문들의 모음입니다. 서버에서 관리 및 주기적으로 업데이트됩니다. |
...
### 2.2. 챌린지 참여 인원 제한 (최대 30명)

하나의 챌린지에 참여할 수 있는 최대 인원을 **30명**으로 제한합니다.

#### 2.2.1. 문제점 (Pain Point)

참여 인원 제한이 없으면 이론적으로 바이럴이 무한히 확장될 것 같지만, 실제로는 다음과 같은 역효과가 발생합니다.

- **결과의 가치 하락**: 200명 중 1명이 나를 지목한 것보다, 30명 중 1명이 지목한 것이 훨씬 더 의미 있고 궁금합니다. 결과의 가치가 떨어지면 유료 결제 전환율도 하락합니다.
- **핵심 재미(상호 지목) 실종**: 참여자 수가 너무 많아지면, 두 사람이 서로를 지목할 확률은 기하급수적으로 낮아집니다. "마음이 통하는" 경험이 줄어들면 사용자의 만족도가 떨어집니다.
- **물리적 시간 한계**: 48시간 내에 30명 이상이 순차적으로 참여하는 것은 현실적으로 어렵습니다. 체인이 중간에 끊길 확률이 높아져, 끝까지 참여하지 못하는 사용자의 부정적인 경험이 늘어납니다.

#### 2.2.2. 해결 방안 (Solution)

- 하나의 챌린지에 참여할 수 있는 **최대 인원을 30명**으로 제한합니다.
- 체인에 **30번째 참여자**가 다음 사람을 지목하면, **"이번 챌린지는 30명이 모두 참여하여 종료되었습니다."** 라는 메시지와 함께 챌린지가 즉시 마감되고 결과가 공개됩니다.

#### 2.2.3. 기대 효과

- **서비스 경험 최적화**: 인류학적 연구(던바의 수)에 따르면, 사람이 의미 있는 관계를 맺는 그룹은 약 15~30명입니다. 이는 학교의 한 반, 친한 친구 그룹의 규모와 일치하여 사용자가 가장 몰입할 수 있는 환경을 제공합니다.
- **상호 지목 확률 증가**: 선택지가 30명으로 좁혀지면서, 서로를 지목할 확률이 유의미하게 높아져 서비스의 핵심 재미를 더 자주 경험할 수 있습니다.
- **결제 전환율 상승**: "30명 중 나를 지목한 사람은 누구일까?" 라는 명확한 범위는 사용자의 궁금증을 극대화하고, 결과를 확인할 만한 충분한 가치를 부여하여 유료 결제를 유도합니다.
- **더 빠른 바이럴**: 바이럴은 하나의 긴 체인이 아니라, 여러 개의 짧은 챌린지가 동시에 퍼져나갈 때 더 효과적입니다. 30명 제한은 사용자가 새로운 챌린지를 더 자주 생성하도록 유도하여, 전체적인 바이럴 확산 속도를 높입니다.
...
| 단계 | 화면 ID | 행위 | 시스템 반응 |
| :--- | :--- | :--- | :--- |
| 1 | - | 사용자가 "누군가 당신을 지목했어요!" 푸시 알림(또는 SMS 딥링크)을 탭합니다. | 앱스토어로 이동하거나 앱이 실행됩니다. |
...
| 9 | CH-02 | 자동으로 자신을 **초대한** 챌린지의 친구 선택 화면으로 이동합니다. 상단에 "누군가 이 질문을 던졌어요!" 문구와 질문이 표시되고, 하단에 친구 목록이 나타납니다. | - |
| 10 | CH-02 | 친구 1명을 선택하고 '차례로 넘기기'를 탭합니다. | 서버에 지목 데이터가 저장되고, 피지목자에게 푸시 알림이 발송됩니다. |
| 11 | HO-02 | 홈 화면으로 이동하며, 참여 중인 챌린지 카드가 표시됩니다. (예: N번째로 참여 완료, OOO 외 M명 참여) | - |

### 3.2. 기존 사용자: 챌린지 생성 (Host·초대)
...
| 3 | CH-INVITE | **첫 참여자 초대** 화면: "이 질문에 답할 친구 3~5명을 골라주세요." 안내와 함께 친구 목록에서 **3명 이상 5명 이하**를 선택합니다. | 선택 인원이 3~5명이면 '초대하기' 버튼이 활성화됩니다. |
| 4 | CH-03 | 확인 팝업: "이 질문으로 OOO, △△△ 외 N명을 초대할까요? (남은 횟수: 2/3)" | '시작하기' 탭 시 챌린지가 생성되고, 선택된 3~5명 **각각**에게 "누군가 이 질문을 던졌어요!" 푸시 알림이 발송됩니다. Host의 챌린지 생성 가능 횟수가 1 차감됩니다. |
| 5 | HO-02 | 홈 화면으로 복귀하며, 방금 생성한 챌린지 카드가 목록 최상단에 추가됩니다. (Host는 "진행 중" 등으로 표시) | - |

### 3.3. 상호 지목 발생 시: 편지 작성 플로우

| 단계 | 화면 ID | 행위 | 시스템 반응 |
| :--- | :--- | :--- | :--- |
| 1 | CH-02 | B가 친구 선택 화면에서 A를 지목합니다. (B는 A가 자신을 지목했다는 사실을 모릅니다.) | 서버에서 상호 지목을 감지합니다. |
| 2 | LT-00 | 상호 지목 축하 화면: "(A의 닉네임)님과 마음이 통했어요!" 라는 애니메이션과 함께 축하 메시지가 표시됩니다. | 3초간 표시 후 자동으로 편지 작성 화면으로 전환됩니다. |
| 3 | LT-01 | 편지 작성 화면: 상단에 안내 문구, 중앙에 텍스트 입력창(500자 제한), 하단에 글자 수 카운터와 '다음' 버튼이 표시됩니다. | 최소 10자 이상 입력해야 '다음' 버튼이 활성화됩니다. |
| 4 | CH-02 | 편지 작성 후 '다음'을 탭하면, 체인을 이어갈 다음 주자를 지목하는 친구 선택 화면으로 이동합니다. (A는 목록에서 제외됩니다.) | - |
| 5 | CH-02 | 다음 주자(D)를 선택하고 '보내기'를 탭합니다. | 편지가 서버에 저장되고, D에게 푸시 알림이 발송됩니다. A에게는 상호 지목 사실이 즉시 알려지지 않으며, 48시간 후 결과 확인 시점에 편지를 확인할 수 있습니다. |

### 3.4. 결과 확인 및 결제 플로우

| 단계 | 화면 ID | 행위 | 시스템 반응 |
| :--- | :--- | :--- | :--- |
| 1 | - | 48시간 경과 후 "결과가 도착했습니다!" 푸시 알림을 수신합니다. | - |
...
하나의 챌린지는 다음과 같은 생명주기를 가집니다.

```mermaid
stateDiagram-v2
    [*] --> Created : 생성자가 챌린지 생성
    Created --> Active : 피지목자에게 알림 발송 완료
    Active --> Active : 체인 내 지목이 계속 이어짐
    Active --> Expired : 48시간 경과 (시스템 자동 처리)
    Active --> Expired : 참여자 30명 도달 (30번째 지목 완료 시 즉시 종료)
    Expired --> ResultDelivered : 모든 참여자에게 결과 알림 발송
    ResultDelivered --> [*]
```

### 4.2. 참여자 개인 상태

챌린지 체인 내 각 참여자(Participant)는 다음과 같은 상태를 가집니다.

```mermaid
stateDiagram-v2
    [*] --> Idle : 앱 설치 완료, 미참여 상태

    Idle --> Nominated : 다른 참여자에게 지목됨 (푸시 알림 수신)
    ...
    Completed_Normal --> WaitingResult : 48시간 대기 중
    Completed_Mutual --> WaitingResult : 48시간 대기 중
    
    WaitingResult --> ResultAvailable : 챌린지 종료, 결과 확인 가능
    ResultAvailable --> [*]
...
| **챌린지 카드 구성** | 질문 내용(1줄, 말줄임 처리). **남은 시간**: "47:12:05 남음" 등 **강조 표시**(큰 글씨·아이콘·색상 등). 참여자 표시: **"OOO 외 11명 참여"** 형태(친구 이름 1명 + 외 N명). 내 상태 뱃지: '내 차례!' (지목 미완료 시, 강조 색상) 또는 **'N번째로 참여 완료'** (지목 완료 시, 회색·서브 텍스트). |
| **동작** | '내 차례!' 뱃지가 있는 카드 탭 → CH-02(친구 지목)로 이동. 'N번째로 참여 완료' 카드 탭 → 챌린지 상세 정보(참여자, 남은 시간 등) 확인 화면. |
...
#### CH-INVITE. 첫 참여자 초대 (Host 전용)

| 항목 | 상세 |
| :--- | :--- |
| **구성 요소** | 상단: "이 질문에 답할 친구 3~5명을 골라주세요." 안내. 선택된 질문 텍스트. 검색 바, 친구 목록(다중 선택). 하단 CTA: 'OOO 외 N명 초대하기' (3~5명 선택 시 활성화). |
| **제한** | 3명 이상 5명 이하만 선택 가능. 자기 자신 제외. 동일 챌린지 내 이미 참여 중인 친구는 '이미 참여 중' 표시 및 선택 불가. |

#### CH-02. 친구 지목 (초대받은 사람·체인 참여자)

| 항목 | 상세 |
| :--- | :--- |
| **구성 요소** | 상단 고정 영역: **(초대 수신 시) "누군가 이 질문을 던졌어요!"** + 질문 텍스트 + 남은 시간. 검색 바: "이름으로 검색". 친구 목록(세로 스크롤): 프로필 사진(원형, 36px), 이름, 전화번호 뒷자리(선택적). 하단 CTA 버튼: 'OOO님 차례로 넘기기' (친구 선택 시 활성화). |
| **목록 정렬** | 기본: 가나다순. Heart to Hearts 가입 유저 우선 표시(상단 섹션 분리: "Heart to Hearts 친구" / "주소록 친구"). 미가입 친구를 지목할 경우, 해당 친구에게 SMS로 앱 설치 링크가 발송됩니다. |
| **제한 사항** | 자기 자신은 목록에서 제외됩니다. 동일 챌린지 내에서 이미 참여한 사람은 재지목할 수 없습니다(목록에서 '이미 참여 중' 표시). |

#### CH-03. 챌린지 시작(초대) 확인 팝업

| 항목 | 상세 |
| :--- | :--- |
| **구성 요소** | 모달 팝업. 타이틀: "챌린지를 시작할까요?". 내용: "질문: [선택한 질문 텍스트]" + "초대할 친구: [이름1], [이름2] 외 N명" + "남은 생성 횟수: X/3". '시작하기' 버튼, '취소' 버튼. |
| **동작** | '시작하기' 탭 → 챌린지 생성 API 호출(초대자 3~5명 ID 전달) → 성공 시 선택된 3~5명 각각에게 "누군가 이 질문을 던졌어요!" 푸시 발송 → HO-02로 이동 + 성공 토스트. '취소' 탭 → 팝업 닫힘, CH-INVITE 유지. |
...
| **동작** | '다음' 탭 → CH-02(다음 주자 지목)로 이동. 이때 상호 지목 상대(A)는 친구 목록에서 제외됩니다. |
...
erDiagram
    ...
    CHALLENGES {
        ...
        varchar status "상태 (active, expired, result_delivered)"
        int participant_count "누적 참여자 수 (최대 30, 도달 시 챌린지 즉시 종료)"
        datetime created_at "생성일시"
...
| Method | Endpoint | Request Body | Response | 설명 |
| :--- | :--- | :--- | :--- | :--- |
| POST | `/v1/challenges` | `{ "question_id": 1, "invitee_ids": [42, 55, 67] }` (3~5명) | `{ "challenge_id": 100, "expires_at": "2026-02-25T14:00:00Z" }` | 챌린지 생성(Host). invitee_ids 3~5명 필수. 생성 잔여 횟수 1 차감. 잔여 0이면 403. |
| GET | `/v1/challenges/active` | - | `{ "challenges": [{ "id", "question_text", "status", "my_status", "my_participant_order", "participant_count" (최대 30), "participant_display" (예: "서준 외 7명"), "expires_at" }] }` | 내가 참여 중인 활성 챌린지 목록. participant_count가 30이면 챌린지 종료·결과 공개. |
| GET | `/v1/challenges/{id}` | - | `{ "id", "question_text", "status", "participant_count", "chain": [...], "expires_at" }` | 특정 챌린지 상세 조회. |

### 8.5. 지목 (Nominations)

| Method | Endpoint | Request Body | Response | 설명 |
| :--- | :--- | :--- | :--- | :--- |
| POST | `/v1/nominations` | `{ "challenge_id": 100, "nominee_id": 55 }` | `{ "nomination_id": 200, "is_mutual": false }` 또는 `{ "nomination_id": 200, "is_mutual": true, "mutual_target_name": "김OO" }` | 친구 지목. 서버에서 상호 지목 여부를 판별하여 응답. `is_mutual: true`이면 클라이언트는 편지 작성 플로우로 전환. |
...
| EC-CH-02 | A→B→C 순으로 지목했는데, C가 A를 지목함 (사이클 발생) | C와 A 사이에 상호 지목이 성립됩니다. C는 A에게 편지를 작성하고, 다시 새로운 친구 D를 지목하여 체인을 이어갑니다. 사이클 자체는 정상적인 시나리오로 처리합니다. |
...
| EC-CH-07 | 챌린지 참여 인원이 30명에 도달함 | 30번째 참여자가 다음 주자를 지목한 시점에 챌린지를 즉시 종료합니다. "이번 챌린지는 30명이 모두 참여하여 종료되었습니다." 메시지를 표시하고, 결과를 공개합니다. 이후 해당 챌린지에는 새 지목을 받을 수 없습니다. |
...
| EC-PAY-02 | '전체 확인' 결제 후 새로운 편지가 도착함 (동일 챌린지) | '전체 확인'은 결제 시점 기준으로 존재하는 모든 편지에 대해 적용됩니다. 결제 이후 도착한 편지는 별도로 확인해야 합니다. (단, 48시간 종료 후 결과가 일괄 공개되므로 이 케이스는 발생 빈도가 매우 낮습니다.) |
...
Heart to Hearts의 성장은 사용자의 자발적인 초대와 참여로 완성되는 바이럴 루프에 절대적으로 의존합니다. 서비스의 핵심 메커니즘 자체가 바이럴 루프로 설계되어 있다는 점이 가장 큰 강점입니다.

| 단계 | 명칭 | 설명 | 핵심 지표 |
| :--- | :--- | :--- | :--- |
| 1 | **초대 (Invite)** | Host(A)가 챌린지를 생성하며 **첫 참여자 3~5명(B, C, D, E)**을 동시에 초대합니다. 한 번의 생성으로 3~5명에게 동시에 "누군가 이 질문을 던졌어요!" 알림이 가므로, 바이럴 계수가 기존(1명) 대비 3~5배로 증가합니다. | 챌린지당 초대 인원 수, 갈래 수 |
| 2 | **인지 (Awareness)** | B, C, D, E는 "누군가 이 질문을 던졌어요!" 푸시 알림을 통해 서비스를 인지합니다. (일반 지목 시에는 "누군가 당신을 지목했어요!") | 알림 클릭률 (CTR) |
| 3 | **전환 (Conversion)** | 초대받은 사람은 궁금증을 해소하기 위해 앱을 설치(신규) 또는 실행(기존)합니다. | 앱 설치 전환율 |
| 4 | **참여 (Action)** | B, C, D, E는 각자 질문에 답하며 다음 주자를 지목합니다. 각 갈래에서 체인이 뻗어 나가며, 상호 지목 발생 확률이 높아집니다. | 지목 완료율 |
| 5 | **보상 (Reward)** | 48시간 후, 참여자들은 자신에 대한 편지를 확인하며 긍정적 경험을 얻습니다. 이 경험은 새로운 챌린지(Host) 생성 동기가 됩니다. | 결과 확인률, 재참여율 |

> **핵심 전략**: Host가 3~5명을 동시에 초대하는 구조로 **초대 수**가 3~5배가 되어 바이럴 계수(K-Factor)를 크게 높일 수 있습니다. 푸시 문구("누군가 이 질문을 던졌어요!")와 온보딩 간결함(3초 이내 핵심 가치 전달 [1]), 첫 지목까지 단계 수 최소화가 핵심입니다.
...
| **챌린지 생성 제한 (3회)** | 희소성은 가치를 높입니다. "나에게 남은 3번의 기회 중 하나를 사용한다"는 인식은 챌린지 참여의 진지함과 특별함을 부여하고, 사용자가 더 신중하게 친구를 선택하도록 유도합니다. |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| PN-01 | 새 챌린지에 **초대**됨 (Host가 3~5명 지정) | 초대받은 사람 | 누군가 이 질문을 던졌어요! | 지금 바로 확인하고 당신의 마음을 이어주세요. 48시간 후 모든 것이 공개됩니다! | 챌린지 참여 화면 (CH-02) |
| PN-01b | 체인에서 지목됨 (초대가 아닌 일반 지목) | 피지목자 | 누군가 당신을 지목했어요! | 지금 바로 확인하고 당신의 마음을 이어주세요. 48시간 후 모든 것이 공개됩니다! | 챌린지 참여 화면 (CH-02) |
| PN-02 | 상호 지목 발생 | 최초 지목자 (A) | 마음이 통했어요! | 누군가 당신을 다시 지목했어요. 48시간 후 특별한 편지가 도착할지도 몰라요! | 홈 화면 (HO-02) |
| PN-03 | 지목 후 6시간 미참여 | 피지목자 | 아직 고민 중인가요? | 당신을 기다리는 사람이 있어요. 지금 마음을 전해보세요! | 챌린지 참여 화면 (CH-02) |
| PN-04 | 지목 후 24시간 미참여 | 피지목자 | 체인이 끊어지기 전에! | 당신의 선택이 다음 이야기를 만들어요. 서둘러주세요! | 챌린지 참여 화면 (CH-02) |
| PN-05 | 챌린지 종료 12시간 전 | 미참여 피지목자 | 챌린지 종료까지 12시간! | 아직 마음을 정하지 못했다면 서두르세요! 곧 결과가 공개됩니다. | 챌린지 참여 화면 (CH-02) |
| PN-06 | 챌린지 종료 1시간 전 | 미참여 피지목자 | 마감 임박! | 마지막 기회를 놓치지 마세요! 1시간 뒤에 모든 결과가 도착합니다. | 챌린지 참여 화면 (CH-02) |
| PN-07 | 챌린지 종료 (결과 도착) | 모든 참여자 | 결과가 도착했습니다! | 나를 지목한 사람은 누구일까요? 지금 바로 보관함에서 확인해보세요! | 결과 보관함 (RE-01) |
...
| **평균 체인 길이** | 하나의 챌린지에서 체인이 이어진 평균 참여자 수 | 8명 이상 |
...
| 단계 | 시기 | 기능 | 설명 |
| :--- | :--- | :--- | :--- |
| **Phase 1 (MVP)** | 런칭 | 핵심 기능 | 챌린지 생성/참여, 꼬리물기 지목, 상호 지목 편지, 결과 확인, 신원 공개 결제. 이 문서에 기술된 모든 핵심 기능. |
